...
当前阶段已经完成基础张量与 CPU 算子闭环，下一步进入 CUDA backend 骨架。
...
- 创建 CUDA backend 骨架。
- 保持 CUDA 为可选能力，没有 CUDA toolkit 时 CPU-only 构建不受影响。
...
当前状态：已完成 `Status`、`Tensor`、`Backend` 抽象、`CPU Backend`、`Engine Runtime`、总入口、示例程序、smoke test、`CPU add`、`CPU matmul`、`CPU softmax` 和 `CPU rms_norm` 算子，下一步是 CUDA backend 骨架。
