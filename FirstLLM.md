...
当前阶段已完成自回归单步生成接口首版：可以调用 Tiny Llama-like forward 得到概率分布，用 greedy argmax 选出 next token，并更新生成状态。下一步进入第 14 章 tokenizer、sampler 和真实文本生成准备。
...
- `CUDA backend` 最小测试。
- `GGUF header reader`。
- `GGUF header reader` 最小测试。
- `GGUF metadata key/value reader`。
- `GGUF metadata key/value reader` 最小测试。
- `GGUF tensor info reader`。
- `GGUF tensor info reader` 最小测试。
- `GGUF data section` 起点计算。
- `GGUF tensor` 数据绝对偏移计算。
- `GGUF data section` 和 tensor 偏移测试。
- `TinyLlamaConfig`、`TinyLlamaWeights` 和 `TinyLlamaModel`。
- `TinyLlamaModel::forward()` 极简前向路径。
- `Tiny Llama-like forward` 最小测试。
- `KvCacheConfig` 和 `KvCache`。
- `KV cache` 追加、读取、清空和边界检查测试。
- `GeneratorConfig` 和 `GenerationState`。
- `GreedyNextToken()` 与 `GenerateOneStep()`。
- `greedy` 单步生成测试。
- `firstllm` 静态库目标。
- CTest 测试闭环。
...
- 进入第 14 章 tokenizer、sampler 和真实文本生成准备。
- 先实现一个最小 sampler 或 token 映射，不急着接真实大模型。
...
当前状态：已完成 `Status`、`Tensor`、`Backend` 抽象、`CPU Backend`、`CUDA Backend` 骨架、`Engine Runtime`、总入口、示例程序、smoke test、`CPU add`、`CPU matmul`、`CPU softmax`、`CPU rms_norm` 算子、GGUF header reader、GGUF metadata key/value reader、GGUF tensor info reader、GGUF data section 对齐、tensor 绝对偏移计算、Tiny Llama-like forward 首版、KV cache 首版和自回归单步生成接口，下一步进入 tokenizer、sampler 和真实文本生成准备。
