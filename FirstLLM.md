...
当前阶段已经进入 GGUF metadata reader，已完成 GGUF 文件头和 metadata key/value 读取，下一步解析 tensor info。
...
- 继续 GGUF metadata reader。
- 读取 tensor info 的基础结构，不急着完整加载权重。
...
当前状态：已完成 `Status`、`Tensor`、`Backend` 抽象、`CPU Backend`、`CUDA Backend` 骨架、`Engine Runtime`、总入口、示例程序、smoke test、`CPU add`、`CPU matmul`、`CPU softmax`、`CPU rms_norm` 算子、GGUF header reader 和 GGUF metadata key/value reader，下一步是 GGUF tensor info 读取。
