...
当前阶段已完成 Tiny Llama-like forward 首版：可以用内存中的 float32 权重串联 RMSNorm、MatMul、Add bias 和 Softmax，形成一个最小模型层前向闭环。下一步进入第 13 章 KV cache 与自回归生成。
...
- 进入第 13 章 KV cache 与自回归生成。
- 先设计 KV cache 的最小数据结构和单步生成接口，不急着做高性能 attention。
...
当前状态：已完成 `Status`、`Tensor`、`Backend` 抽象、`CPU Backend`、`CUDA Backend` 骨架、`Engine Runtime`、总入口、示例程序、smoke test、`CPU add`、`CPU matmul`、`CPU softmax`、`CPU rms_norm` 算子、GGUF header reader、GGUF metadata key/value reader、GGUF tensor info reader、GGUF data section 对齐、tensor 绝对偏移计算和 Tiny Llama-like forward 首版，下一步进入 KV cache 与自回归生成。
