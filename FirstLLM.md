...
- 创建 RMSNorm。
- 为后续 Llama-like forward 准备基础归一化算子。
...
当前状态：已完成 `Status`、`Tensor`、`Backend` 抽象、`CPU Backend`、`Engine Runtime`、总入口、示例程序、smoke test、`CPU add`、`CPU matmul` 和 `CPU softmax` 算子，下一步是 `rms_norm`。
