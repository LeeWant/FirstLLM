...
- 创建 softmax 与 rms_norm。
- 为后续 Llama-like forward 准备基础归一化和概率分布算子。
...
当前状态：已完成 `Status`、`Tensor`、`Backend` 抽象、`CPU Backend`、`Engine Runtime`、总入口、示例程序、smoke test、`CPU add` 和 `CPU matmul` 算子，下一步是 `softmax` 与 `rms_norm`。
